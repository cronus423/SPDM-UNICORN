                         ...
                         size_t address_begin)
{
    ...
    uc_mem_write(uc_sub, address_begin, &length, sizeof(length));
    uc_mem_read(uc_sub, address_begin, &length_read, sizeof(length));
    if (!result) {
        return result;
    }
    uint8_t* buffer_read = (uint8_t*)malloc(length);;
    printf("Platform port Receive size: ");
    length_read = ntohl(length_read);
    dump_data((uint8_t *)&length_read, sizeof(uint32_t));
    printf("\n");
    length_read = ntohl(length_read);
    ...
    *bytes_received = length_read;
    ...
    result = read_bytes(socket, buffer, length_read);
    if (!result) {
        return result;
    }
    uc_mem_write(uc_sub, address_begin + sizeof(length), buffer, length);
    uc_mem_read(uc_sub, address_begin + sizeof(length), buffer_read, length);
    printf("Platform port Receive buffer:\n    ");
    dump_data(buffer_read, length_read);
    printf("\n");
                           ...
                           uc_engine* uc)
{
    ...
    uint32_t response_read;
    uint32_t transport_type_read;
    //uint32_t bytes_received_read;
    ...
    uc_mem_write(uc, ADDRESS_Receive, &response, sizeof(response));
    uc_mem_read(uc, ADDRESS_Receive, &response_read, sizeof(response));

    *command = response_read;
    //*command = response;
    printf("Platform port Receive command: ");

    response_read = ntohl(response_read);
    //response = ntohl(response);
    dump_data((uint8_t *)&response_read, sizeof(uint32_t));
    //dump_data((uint8_t*)&response, sizeof(uint32_t));
    printf("\n");

    result = read_data32(socket, &transport_type);

    uc_mem_write(uc, ADDRESS_Receive+sizeof(response), &transport_type, sizeof(transport_type));
    uc_mem_read(uc, ADDRESS_Receive+sizeof(response), &transport_type_read, sizeof(transport_type));

    if (!result) {
        return result;
    }
    printf("Platform port Receive transport_type: ");
    transport_type_read = ntohl(transport_type_read);
    //transport_type = ntohl(transport_type);
    dump_data((uint8_t *)&transport_type_read, sizeof(uint32_t));
    //dump_data((uint8_t*)&transport_type, sizeof(uint32_t));
    printf("\n");
    
    transport_type_read = ntohl(transport_type_read);
    if (transport_type_read != m_use_transport_layer) {
    ...
    result = read_multiple_bytes(socket, receive_buffer, &bytes_received,
                                 (uint32_t)*bytes_to_receive, uc, ADDRESS_Receive + sizeof(response)+ sizeof(transport_type));
    if (!result) {
        return result;
    }
