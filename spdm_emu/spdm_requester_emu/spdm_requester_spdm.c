...
void *spdm_client_init(void)
{
    ...
    context_size = libspdm_get_context_size();//modified part
    memory_size = ((context_size / 1024) + 1) * 1024;
    m_spdm_context = (void*)malloc(context_size);//modified part
    //m_spdm_context = (void *)malloc(libspdm_get_context_size());
    if (m_spdm_context == NULL) {
        return NULL;
    }
    spdm_context = m_spdm_context;
    uc_mem_map_ptr(uc_requester, ADDRESS_Context, memory_size, UC_PROT_ALL, spdm_context);
